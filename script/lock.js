...
function getOneMoreTin(lockPass) {
    ...
    RemoveWalletLockBTN.onclick = () => {
        chrome.storage.sync.get(['lock'], (lockNew) => {
            ...
            document.getElementsByClassName('remWalInputHid')[0].style.display = 'block'
...
setPinCode.onclick = () => {
    ...
    if (securePin.value === securePin_comfim.value && securePin.value.length > 0 && securePin_comfim.value.length > 0) {
...
function secPageHideShow(data) {
    ...
    remWalInputHid.style.display = (data === 'showSetPassScreen') ? 'block' : 'none';
    secImgContLock.style.display = (data === 'showSetPassScreen') ? 'block' : 'none';
    LockUnset.style.display = (data === 'showSetPassScreen') ? 'block' : 'none';
    LockSet.style.display = (data === 'showSetPassScreen') ? 'none' : 'block';
    WalletLockedImg.style.display = (data === 'showSetPassScreen') ? 'none' : 'block';
    SetSEcPAs.innerText = (data === 'showSetPassScreen') ? 'Set Wallet Password' : 'Wallet Lock Enabled';
}
