...
myDropxco345g.onclick = () => { document.getElementById('myFunction').style.border = 'none' }
...
send_btn_btc.onclick = function () {
    ...
    } else { document.getElementById('myFunction').style.border = 'none'; }
    ...
    var fromAddress = document.getElementById('replacewithaddr').firstChild.innerText;
    ...
    var btcpiricecl = document.getElementsByClassName('btcpiricecl')[0].innerHTML;
...
console.log(newtxString)
                ...
                function sendToserver() {
                    ...
                    con.onreadystatechange = function () {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            con2.onreadystatechange = function () {
                                if (this.readyState == 4) {
                                   ...
                                   console.log(shopPopupSucces1) //After tx json
                                    ...
                                    var usdbtcpricl = document.getElementsByClassName('usdbtcpricl')[0];
                                    var tickerUSD = document.getElementById('btcticker').innerHTML;
                                    var updateBal = shopPopupSucces1.tx.outputs[1].value;

                                    btcpiricecl.innerHTML = updateBal / 100000000 + " BTC";
                                    usdbtcpricl.innerHTML = ((updateBal / 100000000) * tickerUSD).toFixed(2);


                                    var btcval = shopPopupSucces1.tx.outputs[0].value / 100000000;
                                    ...
                                    var popaftersuccestx = document.getElementById('popaftersuccestx');
                                    popaftersuccestx.style.display = 'flex';
                                    popaftersuccestxheaderCont.innerHTML = '<div id="btcusdcont987d"><span id="sucid01">You sent:</span><span id="sucid02">' + btcval + " BTC" + '</span>'
                                        + '<span id="sucid03">' + "$" + USDval + '</span></div><span id="sucid04">To address:</span><span id="sucid05">' + shopPopupSucces1.tx.outputs[0].addresses + '</span><span id="sucid06">Transaction fees:</span>'
                                        + '<span id="sucid07">' + fees + '</span><button id="sucbtncloseinfo34">Ok</button>';
                                    var sucbtncloseinfo34 = document.getElementById('sucbtncloseinfo34');
                            ...
                            con2.open('POST', 'https://api.blockcypher.com/v1/'+network+'/txs/send');
                            con2.send(txfinal);
        ...
        con.open("POST", 'https://api.blockcypher.com/v1/'+network+'/txs/new', true);
