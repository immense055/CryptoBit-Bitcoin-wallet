...
function NetworkToUse(network) {
  ...
  function x1() {
    ...
    e1.innerHTML =
      "<span id='headerz1'>Get info about any bitcoin address</span><span id='closepopggf3'>X</span>";
    ...
    btnsearchp3fr.addEventListener("click", function() {
      if (document.getElementById("getbackinfoid")) {
        document
          .getElementById("xcont")
          .removeChild(document.getElementById("getbackinfoid"));
      }
      ...
      con.onreadystatechange = function() {
        if (this.readyState == 4) {
          ...
          var ar = [
            res.address,
            res.total_sent / 100000000,
            res.total_received / 100000000,
            res.n_tx,
            res.final_balance / 100000000
          ];
          d1.innerHTML =
            '<span class="adrlooupcls98">Address:</span><span class="returndDataxmlg">' +
            ar[0] +
            "</span>" +
            '<span class="adrlooupcls98">Balance:</span><span class="returndDataxmlg">' +
            ar[4] +
            " BTC" +
            "</span>" +
            '<span class="adrlooupcls98">Total received:</span><span class="returndDataxmlg">' +
            ar[2] +
            " BTC" +
            "</span>" +
            '<span class="adrlooupcls98">Total sent:</span><span class="returndDataxmlg">' +
            ar[1] +
            " BTC" +
            "</span>" +
            '<span class="adrlooupcls98">Number of tx:</span><span class="returndDataxmlg">' +
            ar[3] +
            "</span>";
