...
function NetworkToUse(network) {
  ...
  function filltxinfo() {
    ...
    function HandleResponse(ThisResponse) {
      ...
      if (txs.length > 0) {
        document.getElementById("ewtx").innerHTML = "";
      }
            ...
            for (var i2 = 0; i2 < Xaddr.length; i2++) {
              ...
              if (v1 == v2) {
                for (var i3 = 0; i3 < txs[i].outputs.length; i3++) {
                  document.getElementById("bti_tx_info").innerHTML +=
                    '<span class="txContainer"><span class="arrowcontclasRed">' +
                    '<i class="fas fa-minus"></i></span>' +
                    '<span class="valueRecievedGREEN">' +
                    (txs[i].outputs[i3].value / 100000000).toFixed(8) +
                    "</span>" +
                    "<span class='toMiddleWord'>To</span>" +
                    "<span class='txAddrClass'>" +
                    txs[i].outputs[i3].addresses +
                    "</span>" +
                    "</span>";
                }
              }
              for (var e = 0; e < txs[i].outputs.length; e++) {
                if (txs[i].outputs[e].addresses == Xaddr[i2].address) {
                  ...
                  document.getElementById("bti_tx_info").innerHTML +=
                    '<span class="txContainer"><span class="arrowcontclasGrren">' +
                    '<i class="fas fa-plus"></i></span>' +
                    '<span class="valueRecievedRED">' +
                    (txs[i].outputs[e].value / 100000000).toFixed(8) +
                    "</span>" +
                    "<span class='toMiddleWord'>To</span>" +
                    "<span class='txAddrClass'>" +
                    txs[i].outputs[e].addresses +
                    "</span>" +
                    "</span>";
                }
              }
          ...
          chrome.storage.sync.get("addr", f => {
            for (var i = 0; i < f.addr.length; i++) {
              if (f.addr[i].address == aray) {
                var arrowcontclas = document.getElementsByClassName(
                  "arrowcontclas"
                );
                var txAddrClass = document.getElementsByClassName(
                  "txAddrClass"
                );
                var plusBTN = document.getElementsByClassName("plusBTN");
                var valueRecieved = document.getElementsByClassName(
                  "valueRecieved"
                );

                for (var y = 0; y < txAddrClass.length; y++) {
                  if (txAddrClass[y].innerHTML == f.addr[i].address) {
                    arrowcontclas[y].innerHTML =
