...
function NetworkToUse(network) {
    ...
    chrome.storage.sync.get(
      ...
      function(x) {
        ...
        function myFunction() {
          ...
          var check_classlistShow = document
            .getElementById("myDropdown")
            .classList.contains("show");
          if (check_classlistShow) {
                      ...
                      if (this.readyState == 4) {
                        ...
                        if (rert.unconfirmed_balance > 0) {
                          unConfermedBal.innerHTML =
                            "<span class='unConfermedBalText'>Unconfirmed bal:</span><span class='unConfermedBalTextValue'>" +
                            (rert.unconfirmed_balance / 100000000).toFixed(6) +
                            " BTC</span>";
                        }
                        ...
                        crebalspan.innerHTML = getBackfullprice(
                          rert.final_balance
                        );
                        ...
                        function getBackfullprice(x) {
                          ...
                          var a1, a2, a3, a4, a5;
                          var btcvalye = document.getElementById("btcticker")
                            .innerHTML;

                          var pticketbtc = Number(btcvalye);
                          a1 =
                            '<span id="Confirmedbalancetx9876">Confirmed bal:</span><span class="btc_piceInfoo btcpiricecl">' +
                            qwer.toFixed(6) +
                            " BTC</span>";
                          a2 =
                            '<span class="btc_piceInfoo usdbtcpricl">' +
                            (qwer * pticketbtc).toFixed(2) +
                            " USD</span>";
                          a3 = a1 + " " + a2;
                          return a3;
                        }

                        var bitcoinaddressClass = document.getElementsByClassName(
                          "bitcoinaddressClass"
                        );

                        for (var i = 0; i < bitcoinaddressClass.length; i++) {
                          bitcoinaddressClass[i].addEventListener(
                            "click",
                            function(d) {
                              var temp = d.currentTarget.getAttribute(
                                "data-address"
                              );
  ...
  function getbal1() {
    chrome.storage.sync.get("btc_wallet_name", x => {
      ...
      var dashboard_w_tot_balx123 = document.getElementById(
        "dashboard_w_tot_balx123"
      );
